...
def buildAdj(edge_index: Tensor, edge_weight: Tensor, n_node: int, aggr: str):
    ...
    ret = ret.to(edge_index.device) if edge_index.is_cuda else ret
    return ret
...
class PolyConvFrame(nn.Module):
    ...
    def __init__(self,
        ...
        # self.alphas = nn.Parameter(torch.randn(depth+1)) if alphas is None else alphas
        self.cached = cached
        self.aggr = aggr
        self.adj = None
    ...
    def forward1(self, x: Tensor, graph,filter=True):
        '''
        Args:
            x: node embeddings. of shape (number of nodes, node feature dimension)
            edge_index and edge_attr: If the adjacency is cached, they will be ignored.
        ...
        #     alphas = [self.basealpha * torch.tanh(_) for _ in self.alphas]
        alphas = [1 for _ in self.alphas]
        xs = [self.conv_fn(0, [x], graph, alphas)]
        for L in range(1, self.depth + 1):
            tx = self.conv_fn(L, xs, graph, alphas)
            xs.append(tx)
        # if filter:
        #     xs = [self.alphas[i] * x.unsqueeze(1) for i, x in enumerate(xs)]
        # else:
        #     xs = [x.unsqueeze(1) for x in xs]
        xs = [x.unsqueeze(1) for x in xs]
        x = torch.cat(xs, dim=1)
    ...
    def forward2(self, users, graph):
        ...
        '''
        alphas = [1 for _ in self.alphas]
        rows,cols = torch.arange(len(users)),users

        I = torch.zeros((len(users),graph.shape[0])).to(graph.device)
        I[rows,cols] = 1
        xs = [I.T]
        for L in range(1, self.depth + 1):
            tx = self.conv_fn(L, xs, graph, alphas)
            a = tx.to_sparse()
            xs.append(tx)
        # xs = [x.T.unsqueeze(1) for x in xs]
        xs = [self.alphas[i] * x.T.unsqueeze(1) for i, x in enumerate(xs)]
        x = torch.cat(xs, dim=1)
        return x
...
def JacobiConv1(L, xs, adj, alphas, a=1.0, b=1.0, l=-1.0, r=1.0):
    '''
    Jacobi Bases. Please refer to our paper for the form of the bases.
    '''
    if L == 0: return xs[0]
...
def fit_filter(x,alphas):
    y = 0
    for i in range(len(alphas)):
        y += alphas[i]*torch.pow(x,i)
    return y

def high_filter(x,k,i):
    y = 1/(1+torch.exp(k*(x+i)))
    return y

def low_filter(x,k,i):
    y = 1/(1+torch.exp(-k*(x+i)))
    return y
