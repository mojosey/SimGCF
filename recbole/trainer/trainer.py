...
class AbstractTrainer1(object):
    ...
    def __init__(self, config, model1,model2):
        ...
        self.model2 = model2
        if not config["single_spec"]:
            self.model1 = torch.nn.SyncBatchNorm.convert_sync_batchnorm(model1)
            self.distributed_model = DistributedDataParallel(
                self.model1, device_ids=[config["local_rank"]]
            )
...
class Trainer1(AbstractTrainer1):
    ...
    def __init__(self, config, model1,model2):
        super(Trainer1, self).__init__(config, model1,model2)
    ...
    def _build_optimizer(self, **kwargs):
        ...
        """
        params = kwargs.pop("params", self.model1.parameters())
    ...
    def _full_sort_batch_eval(self, batched_data):
        ...
        try:
            ...
            scores1 = self.model1.full_sort_predict(interaction.to(self.device))
            scores2 = self.model2.full_sort_predict2(interaction.to(self.device))
    ...
    @torch.no_grad()
    def evaluate(
        self, eval_data, load_best_model=True, model_file1=None,model_file2=None, show_progress=False
    ):
        r"""Evaluate the model based on the eval data.
        ...
        if load_best_model:
            checkpoint_file1 = model_file1 or self.saved_model_file
            checkpoint = torch.load(checkpoint_file1, map_location=self.device)
            self.model1.load_state_dict(checkpoint["state_dict"])
            self.model1.load_other_parameter(checkpoint.get("other_parameter"))
        ...
        if load_best_model:
            checkpoint_file2 = model_file2 or self.saved_model_file
            checkpoint = torch.load(checkpoint_file2, map_location=self.device)
            self.model2.load_state_dict(checkpoint["state_dict"])
            self.model2.load_other_parameter(checkpoint.get("other_parameter"))
            message_output = "Loading model structure and parameters from {}".format(
                checkpoint_file2
            )
            self.logger.info(message_output)
        ...
        self.model1.alphas = self.model1.low_alphas
        self.model2.alpahs = self.model2.high_alphas
        ...
        self.eval_collector.model_collect(self.model1)
